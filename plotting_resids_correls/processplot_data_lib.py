...
def new_names(dir_=None):
    ...
    if os.path.isfile(path_file):
        ...
        files = ["../data_processing_and_plotting/processed_models_fruity.txt",
                 "../data_processing_and_plotting/processed_models_monash.txt"]
...
def apply_dilution_df(data, kk, zero=0):
    """
    Apply dilution of kk. This formula only works properly for heavy elements
    kk is the fraction of the Ba-star envelope that comes from the AGB
    """
    ...
    # Just apply the formula to each element in name
    new_elements = pd.Series()
    elems_lst = data.keys().values.tolist()
    dilval={}
    for name in elems_lst:
        dilval[name] = np.log10((1 - kk) + kk * 10 ** data[name])
        #new_elements.append(pd.Series(name, dilval))
    dilval = pd.Series(dilval)
...
def get_dict_predicted(files, cm):
    """
        ...
        with open(file_, "r") as fread:
            ...
            for line in fread:
                ...
                # Add this line in fruity or monash
                    ...
                    #lnlst[0] = full_names_dict[lnlst[0]] #+'a'
                    ...
                    dict_[type_][star_name].append(tuple(lnlst))
    return dict_

def conv_dict_to_df(dict):
    '''Converts dictionaries to dataframes'''
    df2 = pd.DataFrame.transpose(pd.DataFrame.from_dict(dict)) # df, still containing strings and "-"-s
    df2 = df2[df2 != "-"].astype(float) # not nan values are float
    return df2


def feature_subtract(df, order=1, plus=False):
    ...
    for a, b in iter.combinations(df.columns, 2):
        if (a and b) != 'Label' and (a or b) not in ['Fe/H', 'Fe/H_err']: # labels are not to be subtracted :)
            ...
            if plus == False: df_new[newcol] = df[a] - df[b]
            else: df_new[newcol] = df[a] + df[b]
            newcols.append(newcol)
    lst_new = df.columns.tolist()
    lst_new.reverse()
    # for a, b in iter.combinations(lst_new, 2):
    #     if (a and b) != 'Label' and (a or b) not in ['Fe/H', 'Fe/H_err']: # labels are not to be subtracted :)
    #         if a[:1] == 'Y':    newcol = '{}/{}'.format(a[:1], b[:2]) # Y is only one character...
    #         elif b[:1] == 'Y':  newcol = '{}/{}'.format(a[:2], b[:1])
    #         else:               newcol = '{}/{}'.format(a[:2], b[:2])
...
def subtract_and_exclude(df_data, toexclude="/Fe"):
    df_data = df_data.transpose()
    df_data = feature_subtract(df_data, 1)
    df_data = df_data[[i for i in df_data.columns if toexclude not in i]]
    df_data.pop('Fe/H')
    return df_data
